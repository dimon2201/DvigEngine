...
void DvigEngine::RenderingSystem::Init()
{
    if (RenderingSystem::m_GlobalVertexBuffer == nullptr)
    {
        ...
        RenderingSystem::m_GlobalUniformBuffer = D3D11::CreateBuffer( D3D11_USAGE_DYNAMIC, D3D11_BIND_SHADER_RESOURCE, D3D11_CPU_ACCESS_WRITE, D3D11_RESOURCE_MISC_BUFFER_STRUCTURED, sizeof(RenderBatchInstanceInfo), nullptr, DVIG_MAX_GFX_BUFFER_BYTE_WIDTH );
        RenderingSystem::m_GlobalConstantBuffer = D3D11::CreateBuffer( D3D11_USAGE_DEFAULT, D3D11_BIND_CONSTANT_BUFFER, 0, 0, 0, nullptr, 2 * sizeof(RenderBatchInstanceInfo) );
        RenderingSystem::m_GlobalUniformBufferView = D3D11::CreateShaderResourceView( RenderingSystem::m_GlobalUniformBuffer, DVIG_MAX_GFX_BUFFER_BYTE_WIDTH, sizeof(RenderBatchInstanceInfo) );
...
void DvigEngine::RenderingSystem::BeginBatch(const RenderBatchInfo* batch)
{
    RenderBatchInfo newBatch;
    newBatch.Geometry = batch->Geometry;
    newBatch.Shader = batch->Shader;
    newBatch.StartVertex = DVIG_NULL;
    newBatch.StartIndex = DVIG_NULL;
    newBatch.StartInstance = DVIG_NULL;

    RenderingSystem::m_Batches->Insert( &newBatch, "new" );
    RenderingSystem::m_GlobalInstanceCount = 0;
}

void DvigEngine::RenderingSystem::Draw(Node* node)
{
    ...
    {
        // Populate batch info
        lastBatch->StartVertex = lastBatch->Geometry->m_GeometryBufferOffset;
        lastBatch->StartIndex = RenderingSystem::m_GlobalInstanceCount;
        lastBatch->InstanceCount = 0;
    }

    // Populate constant buffer
    ViewerComponent* viewer = (ViewerComponent*)RenderingSystem::m_CurRenderPass->Viewer->FindChildByType<ViewerComponent>();
    ...
    TransformComponent* transform = (TransformComponent*)node->FindChildByType<TransformComponent>();
    if (transform == nullptr) { return; }

    const sint32 offset = RenderingSystem::m_GlobalInstanceCount * sizeof(RenderBatchInstanceInfo);
    D3D11::WriteToBuffer( RenderingSystem::m_GlobalUniformBuffer, &transform->m_WorldSpaceMatrix[0][0], offset, sizeof(RenderBatchInstanceInfo) );
